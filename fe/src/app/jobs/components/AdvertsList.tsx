...
const AdvertsList = ({ adverts }: { adverts: AdvertType[] }) => {
  ...
  const [sortDirection, setSortDirection] = useState<sortDirectionValue>(
    sortDirectionValue.DESC
  );
  const [filterByProblemTypes, setFilterByProblemTypes] = useState<
    Array<string>
  >([]);
  const CarProblemTypeLabels = [
    "Engine",
    "Chassis",
    "Body",
    "Tyres",
  ...
  const advertsList = useMemo(() => {
    ...
    if (filterByProblemTypes.length > 0) {
      list = adverts.filter((x) =>
        filterByProblemTypes.includes(x.problemType)
      );
    }
  ...
  const handleSortDirectionChange = () => {
    setSortDirection((state) =>
      state ? sortDirectionValue.ASC : sortDirectionValue.DESC
    );
  };
  ...
  const handleFilterByProblemTypes = (name: string) => {
    ...
    else setFilterByProblemTypes((array) => [...array, name]);
  };
  ...
  return (
    <>
      ...
      <div className="pb-5 flex flex-column justify-between mt-4">
        ...
        <div className="flex flex-column justify-end gap-5">
          <Menu dismiss={{ itemPress: false }}>
            <MenuHandler>
              <Button
                variant="outlined"
                className="flex flex-column gap-2 items-center h-min"
              >
                Filter
                <FaFilter />
              </Button>
                ...
                <MenuItem key={index} className="p-0">
                  ...
                  >
                    <Checkbox
                      ...
                      name={name}
                      onChange={(e) =>
                        handleFilterByProblemTypes(e.target.name)
                      }
                    />
                    {name}
                  </label>
          ...
          <div className="flex flex-column gap-1">
            <Select
              ...
              label="Sort by"
              menuProps={{ className: "shadow-black shadow-sm" }}
            >
